...
import { MagnetogramCreation, MagnetogramMarkupData, MagnetogramResponse, MagnetogramVersion, MagnetogramVersionsComparison, MagnetogramVersionsData } from "../api/http/MagnetogramResponse";
...
export default class MagnetogramStore {
    ...
    async createMagnetogramVersion(formdata: FormData): Promise<MagnetogramVersion> {
        try {
            this.setLoading(true)
            const magnetogram = await MagnetogramService.createMagnetogramVersion(formdata)
            this.setError(false)
            return magnetogram.data
        } catch (e: any) {
            this.setError(true)
