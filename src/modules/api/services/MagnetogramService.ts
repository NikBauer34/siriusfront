...
export default class MagnetogramService {
    ...
    static async getMagnetogramMarkupData(magnetogram_id: string, page: number, bundle: number, i: number): Promise<AxiosResponse<MagnetogramMarkupData>> {
        return $api.get<MagnetogramMarkupData>(`/magnetogram/get_magnetogram_markup_data?id=${magnetogram_id}&page=${page}&bundle=${bundle}&i=${i}`)
    }
